...
library(readxl)
library(dplyr)
#geno_df_imputed <- read.csv("~/R/World_Veg_Project/filtered_data/genotype_matrix_imputed.csv", row.names=1)
...
# IMPORTANT: harmonize SNP column names to match GAPIT (likely "1:17601" format)
# Your genotype columns looked like "X1.17601" earlier
colnames(geno_df_imputed) <- sub("^X", "", colnames(geno_df_imputed))
colnames(geno_df_imputed) <- gsub("\\.", ":", colnames(geno_df_imputed))


# Load metadata + filter annuum
metadata <- readxl::read_excel("~/R/World_Veg_Project/filtered_data/423.xlsx") %>%
  dplyr::rename(Sample_ID = `Sample ID`) %>%
  dplyr::mutate(Sample_ID = trimws(as.character(Sample_ID)))

annuum_ids <- metadata$Sample_ID[metadata$Species == "Capsicum annuum"]

# Subset genotype matrix to annuum-only individuals
annuum_ids_in_geno <- intersect(annuum_ids, rownames(geno_df_imputed))
geno_df_imputed_annuum <- geno_df_imputed[annuum_ids_in_geno, , drop = FALSE]
...
# Load GWAS results 
gwas <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_control/GAPIT_Control_annuum_BLUE_yield/GAPIT.Association.GWAS_Results.FarmCPU.yield(NYC).csv",
  stringsAsFactors = FALSE
)

gwas$P.value <- suppressWarnings(as.numeric(gwas$P.value))
gwas <- gwas[order(gwas$P.value), ]

top_snps <- head(gwas$SNP, 20)


#  Load PCs from GAPIT and standardize ID column name
pcs <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_control/GAPIT_Control_annuum_BLUE_yield/GAPIT.Genotype.PCA.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
)

names(pcs)   # confirm again

# Rename first column manually (simplest and safest)
names(pcs)[1] <- "Taxa"

pcs$Taxa <- trimws(as.character(pcs$Taxa))
...
# phenotype: yield BLUEs (Control)
pheno <- blues_ctrl %>%
  transmute(Taxa = accession,
            y = suppressWarnings(as.numeric(yield))) %>%
  filter(!is.na(y))


# Merge pheno + PCs and restrict to annuum genotypes
base <- pheno %>%
  inner_join(pcs, by = "Taxa") %>%
  filter(Taxa %in% rownames(geno_df_imputed_annuum))

pc_cols <- grep("^PC[0-9]+$", names(base), value = TRUE)[1:5]

# baseline model (PCs only)
f0 <- as.formula(paste("y ~", paste(pc_cols, collapse = " + ")))
r2_base <- summary(lm(f0, data = base))$r.squared


# Compute PVE
pve_tbl <- lapply(top_snps, function(snp) {
  
  if (!snp %in% colnames(geno_df_imputed_annuum)) {
    return(data.frame(SNP = snp, N = NA, R2_marginal = NA, PVE_marginal_pct = NA,
                      R2_delta_PC = NA, PVE_delta_PC_pct = NA,
                      note = "SNP not found in genotype matrix (name mismatch)"))
  }
  
  df <- base %>%
    mutate(g = suppressWarnings(as.numeric(geno_df_imputed_annuum[Taxa, snp]))) %>%
    filter(is.finite(g))
  
  if (nrow(df) < 30) {
    return(data.frame(SNP = snp, N = nrow(df), R2_marginal = NA, PVE_marginal_pct = NA,
                      R2_delta_PC = NA, PVE_delta_PC_pct = NA,
                      note = "N < 30"))
  }
  
  r2_marg <- summary(lm(y ~ g, data = df))$r.squared
  
  f1 <- as.formula(paste("y ~", paste(c(pc_cols, "g"), collapse = " + ")))
  r2_full <- summary(lm(f1, data = df))$r.squared
  
  data.frame(
    SNP = snp,
    N = nrow(df),
    R2_marginal = r2_marg,
    PVE_marginal_pct = 100 * r2_marg,
    R2_delta_PC = r2_full - r2_base,
    PVE_delta_PC_pct = 100 * (r2_full - r2_base),
    note = ""
  )
}) %>% bind_rows()

pve_tbl
write.csv(pve_tbl, "~/R/World_Veg_Project/GWAS_percent_variance/Yield_Control_Top20_PVE.csv", row.names = FALSE)
...
# Load GWAS results 
gwas <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_heat1/GAPIT_Heat1_annuum_BLUE_yield/GAPIT.Association.GWAS_Results.FarmCPU.yield(NYC).csv",
  stringsAsFactors = FALSE
)
...
#  Load PCs from GAPIT and standardize ID column name
pcs <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_heat1/GAPIT_Heat1_annuum_BLUE_yield/GAPIT.Genotype.PCA.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
)
...
pve_tbl
write.csv(pve_tbl, "~/R/World_Veg_Project/GWAS_percent_variance/Yield_Heat1_Top20_PVE.csv", row.names = FALSE)
...
# Load GWAS results 
gwas <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_heat2/GAPIT_Heat2_annuum_BLUE_yield/GAPIT.Association.GWAS_Results.FarmCPU.yield(NYC).csv",
  stringsAsFactors = FALSE
)
...
#  Load PCs from GAPIT and standardize ID column name
pcs <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_heat2/GAPIT_Heat2_annuum_BLUE_yield/GAPIT.Genotype.PCA.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
)
...
pve_tbl
write.csv(pve_tbl, "~/R/World_Veg_Project/GWAS_percent_variance/Yield_Heat2_Top20_PVE.csv", row.names = FALSE)
...
# Load GWAS results 
gwas <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_STI_alltraits/yield/GAPIT_STI_annuum_BLUE_yield_STI_yield/GAPIT.Association.GWAS_Results.FarmCPU.STI_yield(NYC).csv",
  stringsAsFactors = FALSE
)
...
#  Load PCs from GAPIT and standardize ID column name
pcs <- read.csv(
  "/Users/annamccormick/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_STI_alltraits/yield/GAPIT_STI_annuum_BLUE_yield_STI_yield/GAPIT.Genotype.PCA.csv",
  stringsAsFactors = FALSE,
  check.names = FALSE
)
...
# Load BLUEs 
blues_STI <- read.csv(
  "~/R/World_Veg_Project/filtered_data/BLUES/BLUEs_for_GWAS.csv",
  stringsAsFactors = FALSE
)

blues_STI$accession <- trimws(as.character(blues_STI$accession))
...
pve_tbl
write.csv(pve_tbl, "~/R/World_Veg_Project/GWAS_percent_variance/Yield_STI_Top20_PVE.csv", row.names = FALSE)
...
# gene_id is Parent on mRNA, Note is the annotation you want
mrna  <- gff[gff$type == "mRNA"]

collapse_attr <- function(x, sep = "; ") {
  if (is(x, "AtomicList") || is.list(x)) {
    vapply(x, function(z) {
      if (length(z) == 0) return(NA_character_)
      paste(as.character(z), collapse = sep)
...
mrna_note_map <- tibble(
  gene_id = collapse_attr(mcols(mrna)$Parent),
  mrna_id = collapse_attr(mcols(mrna)$ID),
  Note    = collapse_attr(mcols(mrna)$Note)
) %>%
  filter(!is.na(gene_id), gene_id != "") %>%
  group_by(gene_id) %>%
  summarise(
    Note   = Note[which(!is.na(Note) & Note != "")][1],  # first non-empty Note
    mrna_id = mrna_id[1],
    .groups = "drop"
  )
...
# Common fields: ID, Name, gene, gene_name, Parent, product, etc.
head(mcols(genes)[, intersect(names(mcols(genes)), c("ID","Name","gene","gene_name","product","locus_tag","Parent", "Note"))])
...
annotate_snps_with_genes <- function(snp_gr, genes_gr, mrna_note_map) {
  snp_gr <- keepSeqlevels(
    snp_gr,
    intersect(seqlevels(snp_gr), seqlevels(genes_gr)),
    pruning.mode = "coarse"
  )
  ...
  in_gene <- tibble(
    ...
    left_join(mrna_note_map, by = "gene_id")
  ...
  nearest_tbl <- tibble(
    ...
    left_join(mrna_note_map, by = c("nearest_gene_id" = "gene_id")) %>%
    rename(nearest_mrna_id = mrna_id, nearest_Note = Note)
...
ctrl_ann <- annotate_snps_with_genes(ctrl_gr, genes, mrna_note_map)
h1_ann   <- annotate_snps_with_genes(h1_gr,   genes, mrna_note_map)
h2_ann   <- annotate_snps_with_genes(h2_gr,   genes, mrna_note_map)

write.csv(ctrl_ann$in_gene,  "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_control_sig_snps_in_genes_Note.csv", row.names=FALSE)
write.csv(ctrl_ann$nearest,  "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_control_sig_snps_nearest_gene_Note.csv", row.names=FALSE)

write.csv(h1_ann$in_gene,    "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_heat1_sig_snps_in_genes_Note.csv", row.names=FALSE)
write.csv(h1_ann$nearest,    "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_heat1_sig_snps_nearest_gene_Note.csv", row.names=FALSE)

write.csv(h2_ann$in_gene,    "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_heat2_sig_snps_in_genes_Note.csv", row.names=FALSE)
write.csv(h2_ann$nearest,    "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_heat2_sig_snps_nearest_gene_Note.csv", row.names=FALSE)


###### STI
sti_hits   <- read_gapit_hits("~/R/World_Veg_Project/filtered_data/GAPIT_GWAS_annuum_BLUE_STI_alltraits/yield/GAPIT_STI_annuum_BLUE_yield_STI_yield/GAPIT.Association.GWAS_Results.FarmCPU.STI_yield(NYC).csv", p_strict)
sti_gr <- hits_to_gr(sti_hits)
sti_ann <- annotate_snps_with_genes(sti_gr, genes, mrna_note_map)
write.csv(sti_ann$in_gene,  "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_STI_sig_snps_in_genes_Note.csv", row.names=FALSE)
write.csv(sti_ann$nearest,  "~/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/yield_STI_sig_snps_nearest_gene_Note.csv", row.names=FALSE)


##############
# Table S7 SNP annotations 
##############
library(GenomicRanges)
library(rtracklayer)
library(dplyr)
...
to_gff_chr <- function(x) {
  x <- trimws(as.character(x))
  x2 <- gsub("^(?i)chr", "", x, perl = TRUE)   # drop leading chr
  num <- suppressWarnings(as.integer(x2))
  ifelse(!is.na(num), sprintf("Chr%02d", num), x)
}
...
# ----------------------------
gff_path <- "/Users/annamccormick/R/World_Veg_Project/Reference_sequence_cornell/Zhangshugang.gff3"
snp_list_csv <- "/Users/annamccormick/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/table_s7.csv"

out_dir <- "/Users/annamccormick/R/World_Veg_Project/filtered_data/GWAS_HITS_ANNOTATIONS/"
...
# ----------------------------
# Build gene_id -> (mRNA_id, Note) map from mRNA rows
# Note is on mRNA features in your GFF (not on gene features)
# ----------------------------
mrna_note_map <- tibble(
  gene_id = collapse_attr(mcols(mrna)$Parent),
  mrna_id = collapse_attr(mcols(mrna)$ID),
  ...
  mutate(
    ...
    mrna_id = trimws(mrna_id),
    Note    = trimws(Note)
  ) %>%
  filter(!is.na(gene_id), gene_id != "") %>%
  group_by(gene_id) %>%
  summarise(
    mrna_id = mrna_id[which(!is.na(mrna_id) & mrna_id != "")][1],
    Note    = Note[which(!is.na(Note) & Note != "")][1],
    .groups = "drop"
  )

# ----------------------------
# Read SNP list (FORCE SNP to character!!)
# Prevents readr from guessing <time> and turning 1:234 into NA
# ----------------------------
snps <- read_csv(
  snp_list_csv,
  col_types = cols(SNP = col_character())
) %>%
  mutate(SNP = trimws(SNP)) %>%
  filter(!is.na(SNP), SNP != "") %>%
  separate(SNP, into = c("Chr_raw", "Pos_raw"), sep = ":", remove = FALSE, extra = "drop") %>%
  mutate(
    Chr = to_gff_chr(Chr_raw),
    Pos = suppressWarnings(as.integer(Pos_raw))
  ) %>%
  filter(!is.na(Pos))

# ----------------------------
# Convert SNPs -> GRanges
# ----------------------------
snp_gr <- GRanges(
  seqnames = snps$Chr,
...
# Keep only chromosomes present in the gene annotations
snp_gr <- keepSeqlevels(
  snp_gr,
  intersect(seqlevels(snp_gr), seqlevels(genes)),
  pruning.mode = "coarse"
)

# ----------------------------
# Annotate SNPs inside genes (overlap)
# ----------------------------
ov <- findOverlaps(snp_gr, genes, ignore.strand = TRUE)

in_gene <- tibble(
  SNP        = mcols(snp_gr)$SNP[queryHits(ov)],
  Chr        = as.character(seqnames(snp_gr)[queryHits(ov)]),
  Pos        = start(snp_gr)[queryHits(ov)],
  gene_id    = as.character(mcols(genes)$ID[subjectHits(ov)]),
  gene_name  = as.character(if ("Name" %in% names(mcols(genes))) mcols(genes)$Name[subjectHits(ov)] else NA),
  gene_start = start(genes)[subjectHits(ov)],
  gene_end   = end(genes)[subjectHits(ov)]
) %>%
  left_join(mrna_note_map, by = c("gene_id" = "gene_id")) %>%
  rename(mrna_id = mrna_id, Note = Note)

# ----------------------------
# Annotate nearest gene for every SNP (distance 0 if inside)
# ----------------------------
nearest_idx <- nearest(snp_gr, genes, ignore.strand = TRUE)

nearest_tbl <- tibble(
  SNP              = mcols(snp_gr)$SNP,
  Chr              = as.character(seqnames(snp_gr)),
  Pos              = start(snp_gr),
  nearest_gene_id  = as.character(mcols(genes)$ID[nearest_idx]),
  nearest_gene_name = as.character(if ("Name" %in% names(mcols(genes))) mcols(genes)$Name[nearest_idx] else NA),
  nearest_gene_start = start(genes)[nearest_idx],
  nearest_gene_end   = end(genes)[nearest_idx],
  dist_to_nearest_bp = distanceToNearest(snp_gr, genes[nearest_idx], ignore.strand = TRUE)@elementMetadata$distance
) %>%
  left_join(mrna_note_map, by = c("nearest_gene_id" = "gene_id")) %>%
  rename(nearest_mrna_id = mrna_id, nearest_Note = Note)
...
# Write outputs
# ----------------------------
write_csv(in_gene,     file.path(out_dir, "table_s7_snps_in_genes_withNote.csv"))
write_csv(nearest_tbl, file.path(out_dir, "table_s7_snps_nearest_gene_withNote.csv"))
